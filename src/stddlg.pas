...
uses App, Memory, HistList, MsgBox, VpSysLow;

function DriveValid(Drive: Char): Boolean;
begin
  DriveValid := ((1 shl (Ord(Drive) - Ord('A'))) and SysGetValidDrives) <> 0;
end;
...
function DirEntryCompare(item1, item2: Pointer): Integer;
var
  Str1, Str2: string;
...
begin
  Str1 := stUpcase(PDirEntry(item1)^.Directory^);
  Str2 := stUpcase(PDirEntry(item2)^.Directory^);
...
begin
  ...
  begin
    ...
    if OldC <> ' ' then AList^.Insert(NewDirEntry(ldLastDir + OldC, OldC + ':\'));
  end
  else
  begin
    Indent := ldIndentSize;
    ...
    tempColl := New(PDirCollection, Init(5, 5));
    ...
    begin
      ...
      begin
        tempColl^.Insert(NewDirEntry(SR.Name, Dirct + SR.Name));
      end;
      FindNext(SR);
    end;
    FindClose(SR);
    ...
    if tempColl^.Count <> 0 then
      QuickSort(tempColl^.Items, 0, tempColl^.Count - 1, DirEntryCompare);

    isFirst := True;
    for j := 0 to tempColl^.Count - 1 do
    begin
      if isFirst then
      begin
        S := ldFirstDir;
        ...
        S := ldMiddleDir;
      DirEntry := PDirEntry(tempColl^.Items^[j]);
      AList^.Insert(NewDirEntry(Indent + S + DirEntry^.DisplayText^,
        DirEntry^.Directory^));
    end;
